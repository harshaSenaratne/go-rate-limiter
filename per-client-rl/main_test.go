...
func TestSingleAPICall(t *testing.T) {
	...
	// Set fake RemoteAddr to simulate a client IP
...
func TestMultipleAPICallsFromSameClient(t *testing.T) {
	// Test scenario: for i in {1..6}; do curl http://localhost:8080/ping; done
	// This simulates multiple requests from the same client IP
	handler := perClientRateLimiter(endpointHandler)
	
	successCount := 0
	rateLimitedCount := 0
	
	// Make 6 rapid requests from the same client IP
	for i := 1; i <= 6; i++ {
		req := httptest.NewRequest("GET", "/ping", nil)
		// Same IP for all requests to simulate same client
		req.RemoteAddr = "192.168.1.100:12345"
		rr := httptest.NewRecorder()
...
func TestDifferentClientsNotAffected(t *testing.T) {
	// Test that different client IPs have separate rate limits
	handler := perClientRateLimiter(endpointHandler)
	
	// Client 1 makes 4 requests (should exhaust burst)
	for i := 1; i <= 4; i++ {
		req := httptest.NewRequest("GET", "/ping", nil)
		req.RemoteAddr = "192.168.1.100:12345"
		rr := httptest.NewRecorder()
		...
		if rr.Code != http.StatusOK {
			t.Errorf("Client 1 request %d should succeed: got %d", i, rr.Code)
		}
	}
	
	// Client 1's 5th request should be rate limited
	req := httptest.NewRequest("GET", "/ping", nil)
	req.RemoteAddr = "192.168.1.100:12345"
	rr := httptest.NewRecorder()
	handler.ServeHTTP(rr, req)
	...
	if rr.Code != http.StatusTooManyRequests {
		t.Errorf("Client 1's 5th request should be rate limited: got %d", rr.Code)
	}
	...
	if rr2.Code != http.StatusOK {
		t.Errorf("Client 2 should not be affected by Client 1's rate limit: got %d", rr2.Code)
	}
}
